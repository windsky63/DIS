...
def _placement_symbol_anchors(
    ...
    def add_circle(
        ...
        has_x = _has_x_modifier(center, diameter, modifier_lines)
        has_bracket = _has_bracket_modifier(center, diameter, modifier_lines)
        if has_bracket:
...
def _reference_pages(
    ...
    with fitz.open(path) as document:
        for index, page in enumerate(document):
            ...
            if not callouts and not component_callouts:
                continue
            result.append({
...
def _match_design_components_to_reference(
    ...
    design_welds = {
        ...
        if item.get("referenceMatched") and str(item.get("referenceLabel") or "") in reference_welds
    }
    ...
    for component_type in ("valve", "flange", "support"):
        design_group = [item for item in components if item.get("componentType") == component_type]
...
def _primary_page_reference(
    ...
    """Resolve the page's own ISO from the title block, not SEE-ISO notes.

    A page can mention several neighbouring ISO numbers.  The title-block
    identity is consistently the lowest occurrence on the sheet, while
    continuation references occur in the drawing body.  This spatial rule is
    generic across line numbers and avoids filename ordering assumptions.
    """

    ranked: list[tuple[float, float, int, dict[str, Any]]] = []
    weld_references = [reference for reference in references if reference.get("callouts")]
    for reference in weld_references or references:
        stem = Path(str(reference.get("file") or "")).stem
        if not stem:
            continue
        rectangles = page.search_for(stem)
        if not rectangles:
            continue
        lowest = max(rectangles, key=lambda rect: (float(rect.y0), float(rect.x0)))
        ranked.append((float(lowest.y0), float(lowest.x0), len(stem), reference))
    if not ranked:
        return None
...
def _reference_constrained_candidates(
    ...
    *,
    ...
    unfiltered_expected = len(reference_page.get("callouts") or [])
    visually_dashed = [
    ...
    with fitz.open(target_pdf) as document:
        ...
        for page_number in range(first, last + 1):
            ...
            if detection_mode == "comparison":
                ...
                primary_reference = _primary_page_reference(page, references)
                if primary_reference:
                    placement_reference, identity_only_labels = _placement_reference_page(primary_reference)
                    ...
                    reference = dict(reference) | {
                        ...
                        "unfilteredReferenceCalloutCount": len(primary_reference.get("callouts") or []),
                    }
                    ...
                    # Without a page-specific reference, topology-only recovery
                    # points are not authoritative.  Keep the visual composite
                    # signatures and match them against any available references.
                    visual_anchors = [
                    ...
                    reference = _match_reference(page, candidates, references, design_skeleton=design_skeleton)
            ...
            component_reference = primary_reference
            if component_reference is None:
                reference_file = str(reference.get("referenceFile") or "")
                reference_page = int(reference.get("referencePage") or 0)
                component_reference = next((
                    item for item in references
                    if str(item.get("file") or "") == reference_file
                    and int(item.get("page") or 0) == reference_page
                ), None)
            if component_reference is None and len(references) == 1:
                component_reference = references[0]
            component_match_count = _match_design_components_to_reference(
                design_components, weld_candidates, component_reference
            )
        ...
        for page_number, items in grouped.items():
            ...
            for item in items:
                ...
                if shape not in {"circle", "diamond", "rectangle"}:
                    shape = "rectangle"
                frame_size = max(18.0, min(64.0, float(style.get("frameSize") or 30.0)))
