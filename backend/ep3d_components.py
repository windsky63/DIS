...
def _is_component_annotation_color(color: Any) -> bool:
    """Accept the black or red strokes used by EP3D component callouts."""

    if color is None or len(color) < 3:
        return True
    red, green, blue = (float(value) for value in color[:3])
    dark = max(red, green, blue) <= 0.35
    annotation_red = red >= 0.55 and green <= 0.35 and blue <= 0.35
    return dark or annotation_red
...
def _line_primitives(
    ...
    for drawing_index, drawing in enumerate(drawings if drawings is not None else page.get_drawings()):
        ...
        if not _is_component_annotation_color(color):
            continue
...
def extract_design_support_callouts(page: fitz.Page) -> list[Ep3dComponentCallout]:
    ...
    for label, text_bbox in labels:
        ...
        if not enclosing:
            ...
            nearby.sort(key=lambda line: _point_segment_distance(
                center, line["start"], line["end"]
            ))
    ...
    # Two material labels can describe parts of one physical support. Their
    # leader endpoints in the 300900 production drawing are 14.6-14.8 units
    # apart, so a 12-unit radius split each assembly into two candidates.
    support_assembly_merge_distance = 16.0
    ...
    for candidate in sorted(candidates, key=lambda item: item.extraction_confidence, reverse=True):
        duplicate_index = next((
            ...
            if math.dist(item.component_point, candidate.component_point) <= support_assembly_merge_distance
        ), None)
