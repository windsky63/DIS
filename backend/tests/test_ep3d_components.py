...
def _framed_callout(
    ...
    page.insert_text(origin, label, fontsize=10, color=color)
    text_box = page.search_for(label)[-1]
    frame = text_box + (-3, -3, 3, 3)
    page.draw_rect(frame, color=color, width=0.7)
    page.draw_line(
...
class Ep3dComponentTests(unittest.TestCase):
    def test_design_support_frame_uses_nearest_lines_in_dense_area(self) -> None:
        document = fitz.open()
        page = document.new_page(width=500, height=360)
        page.insert_text((200, 100), "S24", fontsize=10, color=(0, 0, 0))
        text_box = page.search_for("S24")[-1]
        frame = text_box + (-3, -3, 3, 3)
        # Put more than the local reconstruction limit into PDF order before
        # the actual frame, reproducing the dense 300900 page-7 neighbourhood.
        for index in range(26):
            y = text_box.y0 - 24 + index * 1.8
            page.draw_line((text_box.x0 - 24, y), (text_box.x0 - 17, y + 0.4), color=(0, 0, 0), width=0.5)
        for start, end in (
            (frame.top_left, frame.top_right),
            (frame.top_right, frame.bottom_right),
            (frame.bottom_right, frame.bottom_left),
            (frame.bottom_left, frame.top_left),
        ):
            page.draw_line(start, end, color=(0, 0, 0), width=0.7)
        target = (270, 170)
        page.draw_line((frame.x1, (frame.y0 + frame.y1) / 2), target, color=(0, 0, 0), width=0.7)
        page.draw_line((260, 164), (280, 164), color=(0, 0, 0), width=0.8)
        page.draw_line((260, 176), (280, 176), color=(0, 0, 0), width=0.8)

        callouts = extract_design_support_callouts(page)
        document.close()

        self.assertEqual([item.label for item in callouts], ["S24"])
    ...
    def test_design_s_frames_follow_leaders_and_merge_multiple_labels_per_support(self) -> None:
        document = fitz.open()
        page = document.new_page(width=500, height=360)
        targets = {"S11": (200, 163), "S13": (200, 177)}
        for label, origin in (("S11", (70, 80)), ("S13", (70, 135))):
            ...
            page.draw_line((frame.x1, (frame.y0 + frame.y1) / 2), targets[label], color=(0, 0, 0), width=0.7)
        ...
        self.assertAlmostEqual(callouts[0].component_point[0], 200, delta=1.0)
    ...
    def test_framed_labels_drive_valve_flange_and_support_identity(self) -> None:
        ...
        _framed_callout(page, "V1", (70, 90), (180, 230), color=(1, 0, 0))
        ...
        _framed_callout(page, "FL2", (220, 80), (278, 230), color=(1, 0, 0))

        for x in (354, 362):
            page.draw_line((x, 220), (x, 240), color=(0, 0, 0), width=0.8)
        _framed_callout(page, "SP3", (350, 90), (358, 230), color=(1, 0, 0))
    ...
    def test_component_inventory_is_ep3d_only_and_does_not_add_design_candidates(self) -> None:
        ...
        self.assertEqual(result["productName"], "图纸标识识别系统")
