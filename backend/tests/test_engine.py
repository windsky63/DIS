...
class EngineTests(unittest.TestCase):
    ...
    def test_title_block_reference_wins_over_see_iso_reference(self) -> None:
        ...
        other = "30-42-0900-26-30DO09002-01"
        main = "30-42-0900-26-30DO09001-01"
        page.insert_text((80, 180), other, fontsize=10)
        page.insert_text((380, 740), main, fontsize=10)
        references = [
            {"file": f"{other}.pdf", "isometric_drawing_no": other, "callouts": []},
            {"file": f"{main}.pdf", "isometric_drawing_no": main, "callouts": []},
        ]
        selected = _primary_page_reference(page, references)
        document.close()
        self.assertIs(selected, references[1])

    def test_page_pairing_allows_only_same_title_number_sheets(self) -> None:
        document = fitz.open()
        page = document.new_page(width=600, height=800)
        identity = "30-42-0900-26-30DO09001-01"
        page.insert_text((380, 740), identity, fontsize=10)
        references = [
            {"file": f"{identity}_sheet_001.pdf", "page": 1,
             "isometric_drawing_no": identity, "identity_source": "reference-title-block"},
            {"file": f"{identity}_sheet_002.pdf", "page": 1,
             "isometric_drawing_no": identity, "identity_source": "reference-title-block"},
            {"file": "30-42-0900-26-30DO09003-01.pdf", "page": 1,
             "isometric_drawing_no": "30-42-0900-26-30DO09003-01"},
        ]
        pairing = _page_reference_pairing(page, references)
        document.close()

        self.assertEqual(pairing["designIsometricDrawingNo"], identity)
        self.assertEqual(pairing["referencePairingStatus"], "exact-title-1:2")
        self.assertEqual(
            [item["file"] for item in pairing["eligibleReferences"]],
            [f"{identity}_sheet_001.pdf", f"{identity}_sheet_002.pdf"],
        )

    def test_reference_page_with_title_identity_survives_empty_object_extraction(self) -> None:
        with tempfile.TemporaryDirectory() as folder_name:
            source = Path(folder_name) / "40-42-3000-26-40DSA30001-01_sheet_002.pdf"
            document = fitz.open()
            page = document.new_page(width=600, height=800)
            page.insert_text((380, 740), "40-42-3000-26-40DSA30001-01", fontsize=10)
            document.save(source)
            document.close()

            references = _reference_pages(source)

        self.assertEqual(len(references), 1)
        self.assertEqual(references[0]["isometric_drawing_no"], "40-42-3000-26-40DSA30001-01")
        self.assertEqual(references[0]["callouts"], [])
        self.assertEqual(references[0]["component_callouts"], [])

    def test_reference_filename_normalization_preserves_intrinsic_double_underscore(self) -> None:
        original = "30-42-0900-26-30NOZ30V0901__LG1-01.pdf"
        stored = Path(f"reference-001__{original}")
        self.assertEqual(_reference_display_name(stored), original)
        self.assertEqual(
            _normalize_isometric_identity(original),
            "30-42-0900-26-30NOZ30V0901/LG1-01",
        )
        self.assertEqual(
            _normalize_isometric_identity("30-42-0900-26-30DO09001-01_sheet_002.pdf"),
            "30-42-0900-26-30DO09001-01",
        )
    ...
    def test_vector_scan_and_annotation_round_trip(self) -> None:
        with tempfile.TemporaryDirectory() as folder_name:
            ...
            with fitz.open(output) as annotated:
                ...
                self.assertIn("weld-marker-source.pdf", annotated.embfile_names())
                ...
                self.assertEqual(annotated.embfile_get("weld-marker-source.pdf"), source.read_bytes())
