...
public static class WeldMarkerJob
{
...
function New-KillOnCloseJob {
    $job = [WeldMarkerJob]::CreateJobObject([IntPtr]::Zero, $null)
    if ($job -eq [IntPtr]::Zero) { throw 'Cannot create the Windows process job.' }

    $information = New-Object WeldMarkerJob+JOBOBJECT_EXTENDED_LIMIT_INFORMATION
    $information.BasicLimitInformation.LimitFlags = [WeldMarkerJob]::JOB_OBJECT_LIMIT_KILL_ON_JOB_CLOSE
    ...
    try {
        ...
        $configured = [WeldMarkerJob]::SetInformationJobObject(
            $job, [WeldMarkerJob]::JobObjectExtendedLimitInformation, $pointer, [uint32]$length
        )
        if (-not $configured) {
        ...
        [void][WeldMarkerJob]::CloseHandle($job)
        throw
    }
    finally {
...
function Add-ProcessToJob([IntPtr]$Job, [Diagnostics.Process]$Process) {
    if (-not [WeldMarkerJob]::AssignProcessToJobObject($Job, $Process.Handle)) {
