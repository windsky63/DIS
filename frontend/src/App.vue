...
import { CANVAS_PERFORMANCE_PROFILES, selectCanvasPerformanceProfile, trimCanvasCache } from './canvasPerformance'
...
let referenceArchiveGeneration = 0
const pageBitmapCache = new Map()
const lazyReferenceFileCache = new Map()
...
const unavailableReferenceFiles = ref([])
...
const tutorialStorageKey = 'weld-marker.tutorial.seen.v1'
const tutorialSteps = Object.freeze([
  ...
  { icon: '▣', title: '使用画布工具栏', description: '工具栏左侧切换页码，中间提供撤销和重做，右侧的 W、V、F、S 分别进入焊口、阀门、法兰和支架修改模式。', tips: ['绿色是普通页码状态；橙色用于标出需要重点确认的页面，绿色不代表该页已经完成确认。', '“适合”可让整张图纸回到最佳可视范围。'], target: '[data-tour="canvas-toolbar"]' },
  ...
  { icon: '+', title: '修改对象与补充遗漏', description: '进入对应类型的修改模式后，可以拖动自动识别对象的锚点调整引线端点。若要补充遗漏，把鼠标移到真实对象位置并按 A，系统会在那里创建人工编号。', tips: ['W、V、F、S 用于选择修改对象类型，A 用于在鼠标位置新增。', '误加对象可在右侧辅助区域排除，或用 Backspace 删除。'], target: '[data-tour="canvas-toolbar"]' },
...
async function openTutorial() {
  ...
  if (!tutorialSessionActive.value && !tutorialSampleLoading.value) await loadTutorialSample()
}
...
function showNotice(message) {
  if (messagesEnabled.value) noticeChannel.publish(message)
}
...
const analysisQueueDialog = ref(false)
const analysisQueueLoading = ref(false)
const analysisQueueActionId = ref('')
const analysisQueueTab = ref('current')
const analysisQueueDeleteTarget = ref(null)
const analysisQueueDeleteDialog = ref(false)
const analysisQueue = ref({ jobs: [], runningCount: 0, queuedCount: 0, maxConcurrent: 1 })
const visibleAnalysisQueueJobs = computed(() => analysisQueue.value.jobs.filter(job => (
  analysisQueueTab.value === 'archived' ? job.isArchived : !job.isArchived
)))
const enqueueing = ref(false)
let healthTimer
let backendAutoSaveTimer
let backendAutoSaveGeneration = 0
let backendSavedGeneration = 0
let backendSavePromise = null
const backendSaveState = ref('idle')
const backendSaveButtonLabel = computed(() => ({
  pending: '待保存',
  saving: '保存中…',
  saved: '已保存',
  error: '重试保存',
})[backendSaveState.value] || '保存')
...
const draftBatchDeleteDialog = ref(false)
const draftBatchDeleting = ref(false)
const draftSelectionMode = ref(false)
const allDraftsSelected = computed(() => draftEntries.value.length > 0 && selectedDraftKeys.value.length === draftEntries.value.length)
const allRenumberConfirmDialog = ref(false)
...
const referenceShowAll = ref(false)
const labelDragState = ref(null)
const canvasLayout = ref({ width: 1, height: 1, target: { x: 0, y: 34, width: 1, height: 1 }, reference: null })
const markerStyle = ref({ shape: 'circle', frameSize: 28, fontSize: 15, lineWidth: 1.35, color: '#d4143c', fillOpacity: 0.58 })
const componentMarkerStyles = ref({
  valve: { shape: 'rectangle', frameSize: 28, fontSize: 15, lineWidth: 1.35, color: '#1769d2', fillOpacity: 0.58 },
  flange: { shape: 'rectangle', frameSize: 28, fontSize: 15, lineWidth: 1.35, color: '#1769d2', fillOpacity: 0.58 },
  support: { shape: 'rectangle', frameSize: 28, fontSize: 15, lineWidth: 1.35, color: '#1769d2', fillOpacity: 0.58 }
})
...
const shortcutDefaults = Object.freeze({ analyze: 'Ctrl+Enter', save: 'Ctrl+S', addWeld: 'W', addValve: 'V', addFlange: 'F', addSupport: 'S', swapWeld: 'X', deleteWeld: 'Backspace' })
function loadShortcutSettings() {
  try {
    ...
    const legacyDeleteDefault = !stored.deleteWeld || stored.deleteWeld === 'Delete'
    return { ...shortcutDefaults, ...stored, ...(legacyAddDefaults ? { addWeld: 'W', addValve: 'V', addFlange: 'F', addSupport: 'S' } : {}), ...(legacyDeleteDefault ? { deleteWeld: 'Backspace' } : {}) }
...
const navigationGroupStart = computed(() => Math.max(0, Math.floor(Math.max(0, navigationIndex.value) / pageButtonsPerGroup.value) * pageButtonsPerGroup.value))
const visibleNavigationPages = computed(() => navigationPages.value.slice(navigationGroupStart.value, navigationGroupStart.value + pageButtonsPerGroup.value))
const canShowPreviousPageGroup = computed(() => navigationGroupStart.value > 0)
const canShowNextPageGroup = computed(() => navigationGroupStart.value + pageButtonsPerGroup.value < navigationPages.value.length)
...
watch(messagesEnabled, value => {
  localStorage.setItem('weld-marker.messages-enabled', String(value))
  if (!value) {
...
watch(error, value => {
  ...
  else if (value && !messagesEnabled.value) error.value = ''
})
...
function historyPageSnapshots(scope = 'page') {
...
function historyInverse(snapshot) {
...
function undo() {
  ...
  redoStack.value.push(historyInverse(snapshot))
...
function redo() {
  ...
  if (!snapshot || !result.value) return
  undoStack.value.push(historyInverse(snapshot))
  applyWorkspaceSnapshot(snapshot)
...
function scheduleDraftSave() {
  ...
  if (!activeFileFingerprint.value || !result.value || result.value.status === 'processing' || applyingHistory.value || hydratingWorkspace.value || archivingWorkspace.value) return
...
function scheduleBackendAutoSave() {
  clearTimeout(backendAutoSaveTimer)
  if (!result.value || result.value.status !== 'complete' || result.value.jobId === 'tutorial-000207' || applyingHistory.value || hydratingWorkspace.value) return
  backendAutoSaveGeneration += 1
  backendSaveState.value = 'pending'
  backendAutoSaveTimer = setTimeout(() => { void persistBackendChanges(false).catch(() => {}) }, 2000)
}

async function persistBackendChanges(notifyUser = false) {
  clearTimeout(backendAutoSaveTimer)
  if (!result.value || result.value.status !== 'complete' || result.value.jobId === 'tutorial-000207') return null
  if (backendSavePromise) {
    await backendSavePromise.catch(() => null)
    if (backendAutoSaveGeneration <= backendSavedGeneration) {
      if (notifyUser) showNotice('当前校对结果已经保存。')
      return null
    }
  }
  const saveGeneration = backendAutoSaveGeneration
  const jobId = result.value.jobId
  const baseRevision = result.value.revision ?? 0
  const savedPages = cloneValue(pages.value)
  const savedAppearance = currentMarkerAppearancePayload()
  backendSaveState.value = 'saving'
  backendSavePromise = api.saveJob(jobId, savedPages, savedAppearance, baseRevision)
  try {
    const saved = await backendSavePromise
    if (result.value?.jobId === jobId) {
      result.value.revision = saved.revision
      result.value.updatedAt = saved.updatedAt
    }
    backendSavedGeneration = Math.max(backendSavedGeneration, saveGeneration)
    backendSaveState.value = backendAutoSaveGeneration > backendSavedGeneration ? 'pending' : 'saved'
    scheduleDraftSave()
    logAudit(notifyUser ? 'job.saved' : 'job.autosaved')
    if (notifyUser) showNotice('当前 PDF 校对结果已保存。')
    return saved
  } catch (cause) {
    backendSaveState.value = 'error'
    if (notifyUser) error.value = cause.message
    else error.value = `自动保存失败，修改仍保留在浏览器草稿中：${cause.message || cause}`
    throw cause
  } finally {
    backendSavePromise = null
    if (backendAutoSaveGeneration > backendSavedGeneration && backendSaveState.value !== 'error') {
      backendAutoSaveTimer = setTimeout(() => { void persistBackendChanges(false).catch(() => {}) }, 500)
    }
  }
}
...
function draftHasStoredTarget(entry) {
  if (entry?.hasStoredTarget !== undefined) return Boolean(entry.hasStoredTarget)
...
async function openDraftManager() {
  ...
  draftSelectionMode.value = false
...
async function openManagedDraft(entry) {
  draftManagerLoading.value = true
  const loadedEntry = await loadDraft(entry?.key).catch(() => null)
  const payload = loadedEntry?.payload
  draftManagerLoading.value = false
...
async function deleteManagedDraft() {
  ...
  try {
    ...
    selectedDraftKeys.value = selectedDraftKeys.value.filter(key => key !== entry.key)
...
function toggleAllDrafts(selected) {
  selectedDraftKeys.value = selected ? draftEntries.value.map(entry => entry.key) : []
}

function toggleDraftSelectionMode() {
  draftSelectionMode.value = !draftSelectionMode.value
  if (!draftSelectionMode.value) selectedDraftKeys.value = []
}

function toggleDraftSelection(key) {
  selectedDraftKeys.value = selectedDraftKeys.value.includes(key)
    ? selectedDraftKeys.value.filter(item => item !== key)
    : [...selectedDraftKeys.value, key]
}

async function deleteSelectedDrafts() {
  const keys = [...selectedDraftKeys.value]
  if (!keys.length || draftBatchDeleting.value) return
  draftBatchDeleting.value = true
  try {
    await Promise.all(keys.map(key => deleteDraft(key)))
    const deleted = new Set(keys)
    draftEntries.value = draftEntries.value.filter(entry => !deleted.has(entry.key))
    selectedDraftKeys.value = []
    draftSelectionMode.value = false
    draftBatchDeleteDialog.value = false
    showNotice(`已删除 ${keys.length} 个浏览器草稿。`)
  } catch (cause) {
    error.value = `批量删除草稿失败：${cause.message || cause}`
  } finally {
    draftBatchDeleting.value = false
  }
}
...
async function refreshServiceHealth() {
  ...
  if (analysisQueueDialog.value) await refreshAnalysisQueue(false)
}

async function refreshAnalysisQueue(showLoading = true) {
  if (showLoading) analysisQueueLoading.value = true
  try {
    const snapshot = await api.getAnalysisQueue()
    analysisQueue.value = {
      ...snapshot,
      jobs: (snapshot.jobs || []).filter(job => job.status !== 'cancelled'),
    }
  } catch (cause) {
    error.value = `读取解析队列失败：${cause.message || cause}`
  } finally {
    if (showLoading) analysisQueueLoading.value = false
  }
}

async function openAnalysisQueue() {
  analysisQueueDialog.value = true
  await refreshAnalysisQueue()
}

function queueStatusLabel(job) {
  if (job.queueState === 'running') return '正在解析'
  if (job.queueState === 'queued') return `等待中 · 第 ${job.queuePosition} 位`
  return ({ complete: '已完成', failed: '失败', cancelled: '已取消', cancelling: '取消中' })[job.status] || '状态未知'
}

function queueStatusColor(job) {
  if (job.queueState === 'running') return 'accent'
  if (job.queueState === 'queued') return 'warning'
  return ({ complete: 'success', failed: 'error', cancelled: 'grey', cancelling: 'warning' })[job.status] || 'grey'
}

function queueProgress(job) {
  return Math.max(0, Math.min(100, (Number(job.completedPages) || 0) / Math.max(1, Number(job.totalPages) || 1) * 100))
}

async function cancelQueueJob(job) {
  if (!job?.canCancel || analysisQueueActionId.value) return
  analysisQueueActionId.value = job.jobId
  try {
    await api.cancelJob(job.jobId)
    await refreshAnalysisQueue(false)
    showNotice(`已请求取消“${job.fileName}”。`)
  } catch (cause) { error.value = cause.message || String(cause) }
  finally { analysisQueueActionId.value = '' }
}

async function moveQueueJob(job, direction) {
  if (!job?.canReorder || analysisQueueActionId.value) return
  analysisQueueActionId.value = job.jobId
  try {
...
async function setQueueJobArchived(job, archived) {
  if (!job?.canArchive || analysisQueueActionId.value) return
  analysisQueueActionId.value = job.jobId
  try {
...
function requestDeleteQueueJob(job) {
  if (!job?.canDelete || result.value?.jobId === job.jobId) return
  analysisQueueDeleteTarget.value = job
  analysisQueueDeleteDialog.value = true
}

async function confirmDeleteQueueJob() {
  const job = analysisQueueDeleteTarget.value
  if (!job || analysisQueueActionId.value) return
  analysisQueueActionId.value = job.jobId
  try {
    await api.deleteJob(job.jobId)
    analysisQueueDeleteDialog.value = false
    analysisQueueDeleteTarget.value = null
    await refreshAnalysisQueue(false)
...
async function restoreQueueJob(job) {
  ...
  try {
    ...
    detectionMode.value = snapshot.symbolConfig?.detectionMode || detectionMode.value
    ...
    if (resultHasMissingNumbers(result.value)) {
      ...
      const saved = await api.saveJob(result.value.jobId, result.value.pages || [], currentMarkerAppearancePayload(), result.value.revision ?? 0)
      result.value.revision = saved.revision
    }
...
onMounted(async () => {
  ...
  try {
    if (localStorage.getItem(tutorialStorageKey) !== 'true') window.setTimeout(openTutorial, 450)
...
function stepPageGroup(direction) {
  const targetIndex = direction > 0
    ? navigationGroupStart.value + pageButtonsPerGroup.value
    : Math.max(0, navigationGroupStart.value - pageButtonsPerGroup.value)
  const next = navigationPages.value[targetIndex]
  if (Number.isFinite(next)) void changePage(next)
}

function navigationPageClasses(pageNumber) {
...
function clearProject() {
  ...
  previewPage.value = 1
  dismissError()
  // Keep the selected reference inputs available for the next drawing, but
  // detach the previous task's rendered/matched page until analysis remaps it.
...
async function loadTutorialSample() {
  ...
  try {
    ...
    clearProject()
    clearReferenceFiles()
    projectPdfs.value = [sample]
    projectResults.value = [tutorialResult]
    targetPdf.value = sample
    referencePdfs.value = tutorialReferences
    ...
    if (resultHasMissingNumbers(result.value)) {
      numberResult(result.value, Math.max(1, Number(startNumber.value) || 1))
      reflowLabelPositions(result.value.pages || [])
    }
    currentPage.value = tutorialStartPage
    previewPage.value = currentPage.value
    projectResults.value[0] = result.value
    referenceResearchReady.value = true
    tutorialSessionActive.value = true
    await nextTick()
    if (referencePdf.value) await syncReferenceForPage(currentPage.value, true)
    leftPanelTab.value = 'input'
    showNotice('000207 教程示例及预解析结果已载入，可直接校对和导出。')
  } catch (cause) {
    error.value = `载入教程示例失败：${cause.message || cause}`
  } finally {
    tutorialSampleLoading.value = false
  }
}

async function loadTargetPdf(file, fitAfter = true, checkRecovery = true, initialPage = 1) {
  ...
  const markPreparationStep = () => {
    ...
    targetPreparation.value = {
      ...targetPreparation.value,
      completed,
      message: completed === targetPreparation.value.total ? '恢复检查完成' : `正在检查可恢复版本（${completed}/${targetPreparation.value.total}）`,
    }
  }
  try {
    const documentPromise = Promise.all([loadPdfjs(), file.arrayBuffer()])
      .then(([pdfjs, data]) => pdfjs.getDocument({ data }).promise)
      .then(document => { markPreparationStep(); return document })
    const fingerprintAndDraftPromise = fileFingerprint(file)
      .then(async fingerprint => ({ fingerprint, draft: checkRecovery ? await loadDraft(fingerprint).catch(() => null) : null }))
      .then(value => { markPreparationStep(); return value })
    const recentPromise = checkRecovery
      ? api.getRecentBatch().catch(() => null).then(value => { markPreparationStep(); return value })
    ...
    const embeddedPromise = checkRecovery
      ? readEditablePdfAttachments(document).then(({ embedded, sourceContent }) => {
          const sourceFile = embedded && sourceContent
            ? new File([sourceContent], embedded.sourceFileName || file.name, { type: 'application/pdf', lastModified: file.lastModified })
            : null
          return { embedded, sourceFile }
        }).catch(() => null).then(value => { markPreparationStep(); return value })
      : Promise.resolve(null)
    ...
    const options = []
    if (embedded) {
      options.push({ key: 'embedded', title: 'PDF 内嵌编辑数据', source: 'embedded', sourceFile: embeddedAssets?.sourceFile || null, priority: 1, payload: { result: embedded, markerStyles: embedded.markerStyles, markerStyle: embedded.markerStyle || markerStyle.value, componentMarkerStyles: embedded.componentMarkerStyles, currentPage: embedded.analyzedRange?.[0] || 1 }, savedAt: '随当前 PDF 保存的可编辑版本' })
    }
    ...
    if (options.length) {
      ...
      targetPreparation.value = { ...targetPreparation.value, active: true, completed: 4, message: '发现可恢复版本，请先选择' }
    } else {
      targetPreparation.value = { ...targetPreparation.value, message: '正在渲染 PDF 页面' }
      await renderUnifiedCanvas(fitAfter)
...
async function restoreWorkspaceDraft() {
  ...
  hydratingWorkspace.value = true
  const loadedTargetFile = targetPdf.value
  if (recovery?.sourceFile) {
    targetPdf.value = recovery.sourceFile
    projectPdfs.value[activeProjectIndex.value] = recovery.sourceFile
  }
  ...
  if (payload.workspaceDraftSchema) result.value.workspaceDraftSchema = payload.workspaceDraftSchema
  ...
  else if ((result.value.referenceFiles || []).length) referenceMode.value = 'pdf'
  if (payload.referenceProjectMode) referenceProjectMode.value = payload.referenceProjectMode
  else if (String(result.value.referenceMode || '').startsWith('reference-pdf-folder')) referenceProjectMode.value = 'folder'
  const restoredReferenceFiles = (payload.referenceFiles || []).map(restoreWorkspaceFile).filter(Boolean)
  const restoredPcfFiles = (await Promise.all((payload.pcfFiles || []).map(materializeStoredWorkspaceFile))).filter(Boolean)
  const expectedReferenceFiles = Array.isArray(result.value.referenceFiles) ? result.value.referenceFiles.filter(Boolean) : []
  let recoveredReferenceFiles = restoredReferenceFiles
  if (!recoveredReferenceFiles.length && expectedReferenceFiles.length && result.value.jobId) {
    recoveredReferenceFiles = await api.getJobReferenceManifest(result.value.jobId).catch(() => [])
  }
  referencePdfs.value = recoveredReferenceFiles
  const recoveredReferenceNames = new Set(recoveredReferenceFiles.map(file => file.name))
  unavailableReferenceFiles.value = expectedReferenceFiles.filter(name => !recoveredReferenceNames.has(name))
  ...
  undoStack.value = []
  redoStack.value = []
  showNotice(unavailableReferenceFiles.value.length
    ? `已恢复标识数据；原对照 PDF 无法自动取回，请重新选择：${unavailableReferenceFiles.value.join('、')}`
    : expectedReferenceFiles.length
      ? '已恢复可编辑标识数据及对照 PDF，可继续校对、移动和导出。'
      : '已恢复可编辑标识数据，可继续校对、移动和导出。')
  ...
  try {
    ...
    else if (recovery?.source !== 'recent-batch' && targetPdf.value === loadedTargetFile) await renderUnifiedCanvas(true)
  } finally {
    await nextTick()
    hydratingWorkspace.value = false
    targetPreparation.value = { ...targetPreparation.value, active: false }
...
async function discardWorkspaceDraft() {
  ...
  targetPreparation.value = { ...targetPreparation.value, active: true, message: '正在渲染 PDF 页面' }
  try { await renderUnifiedCanvas(true) }
  finally { targetPreparation.value = { ...targetPreparation.value, active: false } }
}
...
function clearReferenceDisplay() {
...
function clearReferenceFiles() {
  ...
  referencePdfs.value = []
  referenceResearchReady.value = false
  unavailableReferenceFiles.value = []
}
...
function setReferenceFiles(value) {
  ...
  if (referenceProjectMode.value === 'folder') {
    activeReferenceIndex.value = -1
    referencePdf.value = null
    showNotice(`已登记 ${pdfs.length} 份对照 PDF；完成图元研究后将按匹配关系加载到 Canvas。`)
    void renderUnifiedCanvas(true)
    return
  }
...
function setReferenceFolderFiles(value) {
  ...
  activeReferenceIndex.value = -1
  referencePdf.value = null
  showNotice(`已登记 ${pdfs.length} 份对照 PDF；PCF 请在下方单独选择文件夹。`)
  void renderUnifiedCanvas(true)
}
...
async function jumpToReferenceHint(item) {
  ...
  if (referenceFocus.value?.file === documentInfo.file && referenceFocus.value?.page === pageInfo.page
      && referenceFocus.value?.label === item.label && referenceFocus.value?.type === item.type) {
    referenceFocus.value = null
    return
  }
  referenceShowAll.value = false
  const index = resolveReferenceFileIndex(documentInfo.file, referencePdfs.value)
  ...
  if (!sameReferenceSlot(referencePdf.value, referencePdfs.value[index], index) || !referenceDocument.value) {
...
function showAllReferenceHints() {
  referenceFocus.value = null
  referenceShowAll.value = !referenceShowAll.value
  fitCanvas()
}

function matchedReference(pageNumber) {
  const researchedPage = pages.value.find(page => page.page === pageNumber)
  return resolveMatchedReference(researchedPage, referencePdfs.value)
}
...
async function syncReferenceForPage(pageNumber, fitAfter = true) {
  ...
  const matchedPdf = referencePdfs.value[match.index]
  if (sameReferenceSlot(referencePdf.value, matchedPdf, match.index) && referenceDocument.value) {
    activeReferenceIndex.value = match.index
    await renderUnifiedCanvas(fitAfter)
    return
  }
  activeReferenceIndex.value = match.index
  referencePdf.value = matchedPdf
  await loadReferencePdf(referencePdf.value, fitAfter)
}

async function materializeReferenceFile(file) {
  if (!file || file instanceof Blob) return file
  if (file.storedFileKey) return await materializeStoredWorkspaceFile(file)
  if (!file.lazyReference || !file.url) return file
  const slot = referenceFileSlot(file, file.index)
  if (lazyReferenceFileCache.has(slot)) {
    const cached = lazyReferenceFileCache.get(slot)
    lazyReferenceFileCache.delete(slot)
    lazyReferenceFileCache.set(slot, cached)
    return await cached
  }
  const pending = api.getJobReferenceFile(file)
  lazyReferenceFileCache.set(slot, pending)
  let loaded
  try {
    loaded = await pending
  } catch (cause) {
    if (lazyReferenceFileCache.get(slot) === pending) lazyReferenceFileCache.delete(slot)
    throw cause
  }
  if (lazyReferenceFileCache.get(slot) === pending) lazyReferenceFileCache.set(slot, loaded)
  while (lazyReferenceFileCache.size > MAX_LAZY_REFERENCE_FILES) {
    lazyReferenceFileCache.delete(lazyReferenceFileCache.keys().next().value)
  }
  return loaded
}

async function archiveLazyReferencesForDraft() {
  const descriptors = [...referencePdfs.value]
  const workspaceResult = result.value
  const workspaceResults = [...new Set([workspaceResult, ...projectResults.value].filter(Boolean))]
  ...
  const generation = ++referenceArchiveGeneration
  archivingWorkspace.value = true
  const restored = new Array(descriptors.length)
  let cursor = 0
  const worker = async () => {
    while (cursor < descriptors.length && generation === referenceArchiveGeneration) {
      const index = cursor
      cursor += 1
      restored[index] = await materializeReferenceFile(descriptors[index]).catch(() => descriptors[index])
    }
  }
  ...
  if (generation !== referenceArchiveGeneration) return
  if (descriptors.length) referencePdfs.value = restored
...
function sameReferenceSlot(left, right, fallbackIndex = 0) {
...
async function loadReferencePdf(file, fitAfter = true) {
  ...
  try {
    const resolvedFile = await materializeReferenceFile(file)
    if (token !== referenceLoadToken.value) return
    const previousDocument = referenceDocument.value
    if (previousDocument) {
      renderGeneration += 1
      cancelRenderTasks()
...
function cachePageBitmap(cacheKey, canvas) {
  ...
  trimCanvasCache(pageBitmapCache, canvasPerformanceProfile.value.cacheBytes)
}
...
async function renderPageOffscreen(page, viewport, pixelFactor, cacheKey) {
  ...
  if (persisted?.blob && persisted.width === width && persisted.height === height) {
    ...
    } catch { /* Safari/Chrome bitmap cache decode failed; render the PDF page again below. */ }
  }
...
async function renderUnifiedCanvas(fitAfter = false, indicateLoading = true) {
  if (!targetDocument.value || !pdfCanvas.value) return
  const generation = ++renderGeneration
  cancelRenderTasks()
  if (indicateLoading) rendering.value = true
  try {
    const requestedTargetDocument = targetDocument.value
    const requestedReferenceDocument = referenceDocument.value
    const requestedTargetFile = targetPdf.value
    const requestedReferenceFile = referencePdf.value
    const requestedProjectIndex = activeProjectIndex.value
    const requestedReferenceIndex = activeReferenceIndex.value
    const requestedShownPage = Number(shownPage.value) || 1
    const requestedReferencePage = Number(activeReferencePage.value) || 1
    const pageNumber = Math.max(1, Math.min(requestedTargetDocument.numPages, requestedShownPage))
    const targetPage = await requestedTargetDocument.getPage(pageNumber)
    const referencePageNumber = requestedReferenceDocument ? Math.min(Math.max(1, requestedReferencePage), requestedReferenceDocument.numPages) : 0
    const referencePage = referencePageNumber ? await requestedReferenceDocument.getPage(referencePageNumber) : null
    if (generation !== renderGeneration || requestedTargetDocument !== targetDocument.value || requestedReferenceDocument !== referenceDocument.value || requestedProjectIndex !== activeProjectIndex.value || requestedReferenceIndex !== activeReferenceIndex.value || requestedShownPage !== Number(shownPage.value) || requestedReferencePage !== Number(activeReferencePage.value)) return
    ...
    // Scale the backing canvas to the detected device tier. Safari/iOS remains
    // on the conservative profile when the Device Memory API is unavailable.
    const dimensionLimit = canvasPerformanceProfile.value.maxDimension / Math.max(logicalWidth, logicalHeight)
    const areaLimit = Math.sqrt(canvasPerformanceProfile.value.maxPixels / Math.max(logicalWidth * logicalHeight, 1))
    ...
    const [targetBitmap, referenceBitmap] = await Promise.all([
      renderPageOffscreen(targetPage, targetViewport, pixelFactor, bitmapCacheKey('target', requestedProjectIndex, requestedTargetFile, pageNumber, targetViewport, pixelFactor)),
      referencePage && referenceViewport ? renderPageOffscreen(referencePage, referenceViewport, pixelFactor, bitmapCacheKey('reference', referenceFileSlot(requestedReferenceFile, requestedReferenceIndex), requestedReferenceFile, referencePageNumber, referenceViewport, pixelFactor)) : null
    ])
    if (generation !== renderGeneration || requestedTargetDocument !== targetDocument.value || requestedReferenceDocument !== referenceDocument.value || requestedProjectIndex !== activeProjectIndex.value || requestedReferenceIndex !== activeReferenceIndex.value || requestedShownPage !== Number(shownPage.value) || requestedReferencePage !== Number(activeReferencePage.value)) return
    ...
    if (referenceViewport) {
      ...
      const referenceCaption = referencePdfs.value.length > 1
        ? `对照 PDF ${requestedReferenceIndex + 1}/${referencePdfs.value.length} · P${referencePageNumber} · ${requestedReferenceFile?.name || ''}`
    ...
    // A low-resolution or resize render may supersede a loading render. The
    // newest completed generation always owns clearing the shared spinner.
...
function scheduleResolutionRender() {
  clearTimeout(resolutionTimer)
  resolutionTimer = setTimeout(() => { void renderUnifiedCanvas(false, false) }, 140)
}

function fitCanvas(scheduleRender = true) {
  const viewport = canvasViewport.value
  ...
  if (scheduleRender) scheduleResolutionRender()
}
...
async function changePage(pageNumber) {
  const generation = ++pageChangeGeneration
  ...
  if (result.value?.jobId && requested && !requested.layoutObstacles) {
    try {
      const localPage = activeFileFingerprint.value
        ? await loadDraftPage(activeFileFingerprint.value, activeProjectIndex.value, pageNumber).catch(() => null)
        : null
      if (localPage) Object.assign(requested, localPage, { detailsLoaded: true })
        ...
        Object.assign(requested, hydrated.page || hydrated, { detailsLoaded: true })
      }
    } catch { /* existing light page record remains usable */ }
    if (generation !== pageChangeGeneration) return
    // The full page payload can contain more precise reference matching data.
    if (referenceMode.value === 'pdf') await syncReferenceForPage(pageNumber, false)
    else await renderUnifiedCanvas(false)
  }
}
...
function addManualAtClientPoint(clientX, clientY) {
  ...
  const item = {
    ...
    defaultMarkerStyle: componentType ? { shape: 'rectangle', color: '#1769d2' } : { shape: 'circle', color: '#d4143c' },
    symbolShape: 'manual',
    included: true,
    origin: 'manual',
  ...
  showNotice(`已人工增加${typeTitle} ${item.number}；可分别拖动定位点和标识框调整引线两端。`)
}

function candidateMatchesModificationType(item) {
  return manualAddType.value === 'weld'
    ? !isDesignComponent(item)
    : item.componentType === manualAddType.value
}
...
async function enqueueForReview() {
  if (!targetPdf.value || enqueueing.value) { if (!targetPdf.value) error.value = '请先选择待解析的 ISO PDF 或工程文件夹。'; return }
  enqueueing.value = true
  dismissError()
  clearNotice()
  try {
    const files = projectPdfs.value.length ? projectPdfs.value : [targetPdf.value]
    const [encodedTargets, encodedReferences, encodedPcfs] = await Promise.all([
      Promise.all(files.map(file => encodeFile(file))),
      referenceMode.value === 'pdf' ? Promise.all(referencePdfs.value.map(file => encodeFile(file))) : [],
      referenceMode.value === 'pdf' ? Promise.all(pcfFiles.value.map(file => encodeFile(file))) : [],
    ])
    ...
    for (let index = 0; index < files.length; index += 1) {
      const created = await api.createJob({
        targetPdf: encodedTargets[index],
        ...
        referencePdfs: encodedReferences,
        pcfFiles: encodedPcfs,
    ...
    await refreshAnalysisQueue(false)
    showNotice(`${queued ? `已将 ${queued} 份图纸推入解析队列` : '没有新增等待任务'}${alreadyAvailable ? `；${alreadyAvailable} 份已有可恢复结果` : ''}。`)
  } catch (cause) {
...
async function analyze() {
  ...
  try {
    ...
    if (cause?.name === 'JobCancelled' || cause?.name === 'AnalysisDialogCancelled') showNotice(cause.message)
...
function requestRenumberAll() {
  if (!result.value) return
  allRenumberConfirmDialog.value = true
}

function numberWelds() {
  allRenumberConfirmDialog.value = false
  beginHistory('全部重新编号', 'all')
  numberResult(result.value, Math.max(1, Number(startNumber.value) || 1))
  commitHistory()
  showNotice(`已重新生成 ${activeCandidates.value.length} 个焊口及管件编号。`)
}
...
function renumberFromSelected() {
  ...
  showNotice(`已在第 ${candidate.page} 页以 ${candidate.number || '?'} 为起点，重新编号同类对象 ${rotated.length} 个。`)
}
...
function swapSelectedWeldNumbers() {
  ...
  if (!swapSourceId.value) {
    ...
    showNotice(`已暂存焊口 ${selected.number || '?'}；请选择另一个焊口并再次按 X。`)
    return
  }
  ...
  if (!source) {
    swapSourceId.value = selected.id
    showNotice(`原交换对象已失效，现已暂存焊口 ${selected.number || '?'}。`)
    return
  }
  ...
  commitHistory()
  swapSourceId.value = ''
  showNotice(`已交换焊口 ${source.number || '?'} 与 ${selected.number || '?'} 的编号。`)
}
    ...
    items.forEach(item => {
      ...
      for (let ring = 0; ring < 12; ring += 1) {
        ...
        candidateAngles.forEach((angle, directionIndex) => {
          ...
          let existingLeaderIntrusions = 0
          placed.forEach((other, index) => {
            if (rectanglesOverlap(rectangle, other, 5)) labelCollisions += 1
            if (segmentIntersectsRectangle(leader, other)) leaderLabelCrossings += 1
            if (segmentIntersectsRectangle(placedLeaders[index], rectangle)) existingLeaderIntrusions += 1
          })
          let anchorCollisions = 0
          let leaderAnchorProximity = 0
          anchors.forEach(point => {
            if (point.id === item.id) return
            if (point.x > rectangle.left - 8 && point.x < rectangle.right + 8 && point.y > rectangle.top - 8 && point.y < rectangle.bottom + 8) anchorCollisions += 1
            if (pointToSegmentDistance(point, leader) < 7) leaderAnchorProximity += 1
          })
          const rectangleArea = Math.max(1, dimensions.width * dimensions.height)
          let textOverlapRatio = 0
          let textProximity = 0
          let leaderTextCrossings = 0
          textRectangles.forEach(textRectangle => {
            textOverlapRatio += rectangleIntersectionArea(rectangle, textRectangle) / rectangleArea
            if (rectanglesOverlap(rectangle, textRectangle, 8)) textProximity += 1
            if (segmentIntersectsRectangle(leader, textRectangle)) leaderTextCrossings += 1
          })
          let pipeCollisions = 0
          let pipeProximity = 0
          const pipeClearanceRectangle = { left: rectangle.left - 5, right: rectangle.right + 5, top: rectangle.top - 5, bottom: rectangle.bottom + 5 }
          processSegments.forEach(segment => {
            if (segmentIntersectsRectangle(segment, rectangle)) pipeCollisions += 1
            else if (segmentIntersectsRectangle(segment, pipeClearanceRectangle)) pipeProximity += 1
          })
          ...
          const score = labelCollisions * 10_000_000 + anchorCollisions * 8_000_000
            ...
            + leaderLabelCrossings * 4_000_000 + existingLeaderIntrusions * 4_000_000
            + leaderCrossings * 3_000_000 + leaderTextCrossings * 8_000 + pipeProximity * 90_000
            + leaderAnchorProximity * 1_600_000 + ring * 320 + clampPenalty * 35 + directionIndex
          if (!best || score < best.score) best = { x, y, rectangle, leader, score, hard: labelCollisions + anchorCollisions + leaderLabelCrossings + existingLeaderIntrusions + leaderCrossings + leaderAnchorProximity, textOverlapRatio, pipeCollisions, pipeProximity, textProximity }
        })
        if (best && best.hard === 0 && best.textOverlapRatio === 0 && best.pipeCollisions === 0 && best.pipeProximity === 0 && best.textProximity === 0) break
      }
      if (!best) return
      item.labelX = best.x
      item.labelY = best.y
...
function reflowAllLabelPositions(announce = false) {
  if (!result.value) return
  beginHistory('重新优化标识位置', 'all')
  const totals = reflowLabelPositions(pages.value)
  commitHistory()
  if (announce) showNotice(totals.moved
...
function scheduleMarkerReflow() {
  ...
  markerReflowTimer = setTimeout(() => {
    ...
    if (result.value) showNotice('已按新的标识尺寸重新排布，焊口锚点和引线起点保持不变。')
  }, 180)
}
...
function startAnchorDrag(event, item) {
  if (event.button !== 0 || (item.origin !== 'manual' && !manualAddMode.value)) return
  event.preventDefault()
  event.stopPropagation()
  selectCandidate(item)
  beginHistory(item.origin === 'manual' ? '移动人工标识引线端点' : '调整识别标识引线端点')
...
function deleteSelectedWeld() {
  ...
  commitHistory()
  showNotice(`已删除焊口 ${item.number || '?'}，可使用 Ctrl+Z 撤销。`)
...
<template>
  <v-app class="weld-app" @pointerdown.capture="onApplicationPointerDown">
    <v-app-bar color="#173e57" elevation="3" :height="appHeaderHeight">
      <v-app-bar-title class="ml-5"><div class="app-title"><span class="app-title__icon" aria-hidden="true"><svg viewBox="0 0 32 32"><path d="M5 16h7m8 0h7M12 9v14m8-14v14" /><circle cx="16" cy="16" r="5" /><path d="m13.2 13.2 5.6 5.6m0-5.6-5.6 5.6" /></svg></span><span class="app-title__copy"><span>图纸标识识别系统</span><small>焊口 · 阀门 · 法兰 · 支架拓扑识别</small></span></div></v-app-bar-title>
      <div class="header-icon-area mr-4">
        <button data-tour="analysis-queue-button" type="button" class="header-status-icon header-status-icon--interactive" :class="`status-${health}`" :aria-label="health === 'ready' ? '查看解析队列（图元引擎就绪）' : health === 'offline' ? '查看解析队列（后端未连接）' : '查看解析队列（图元引擎正在连接）'" @click="openAnalysisQueue">
          ...
          <v-tooltip activator="parent" location="bottom">{{ health === 'ready' ? '图元引擎就绪 · 点击查看解析队列' : health === 'offline' ? '后端未连接 · 点击查看解析队列' : '图元引擎正在连接 · 点击查看解析队列' }}</v-tooltip>
        </button>
        ...
        <v-btn data-tour="tutorial-button" class="header-icon-button tutorial-button" icon size="small" aria-label="打开操作教程" @click="openTutorial">
        ...
        <v-card class="shortcut-card" min-width="330">
          ...
          <v-list density="compact">
            ...
            <v-list-item :title="shortcutSettings.addWeld" subtitle="进入或退出焊口修改模式" />
            <v-list-item :title="shortcutSettings.addValve" subtitle="进入或退出阀门修改模式" />
            <v-list-item :title="shortcutSettings.addFlange" subtitle="进入或退出法兰修改模式" />
            <v-list-item :title="shortcutSettings.addSupport" subtitle="进入或退出支架修改模式" />
            <v-list-item title="A" subtitle="在修改模式中，于当前鼠标位置新增对应类型的人工编号" />
        ...
        <v-btn data-tour="settings-button" class="header-icon-button settings-button" icon size="small" aria-label="系统设置" @click="systemSettingsDialog = true"><span class="settings-glyph" aria-hidden="true">⚙</span><v-tooltip activator="parent" location="bottom">系统设置</v-tooltip></v-btn>
        </div>
      </div>
    </v-app-bar>
    ...
    <v-navigation-drawer data-tour="input" permanent :rail="!leftDrawerOpen" :rail-width="24" :width="leftDrawerWidth" class="control-drawer">
      ...
      <template v-else>
        <div class="drawer-heading">
          ...
          <button data-tour="left-panel-toggle" type="button" class="drawer-view-toggle" :class="{ 'drawer-view-toggle--active': leftPanelTab === 'reference' }" :disabled="leftPanelTab === 'input' && !result?.ep3dReferenceInventory?.documents?.length" :aria-label="leftPanelTab === 'reference' ? '切换到输入操作' : '切换到对照图提示'" @click="leftPanelTab = leftPanelTab === 'reference' ? 'input' : 'reference'">
        ...
        <v-btn data-tour="analyze" block color="accent" size="large" :loading="loading" :disabled="!targetPdf || enqueueing" class="primary-action-button mt-2" @click="analyze">{{ detectionMode === 'placement' ? '智能编号' : '开始对照识别' }}<v-tooltip activator="parent">{{ detectionMode === 'placement' ? '识别焊口、阀门、法兰和支架，并按类型自动生成编号' : '识别红色编号框和引线，直接恢复已有焊口号' }}</v-tooltip></v-btn>
        </template>
        <template v-else>
    ...
    <v-main data-tour="canvas" class="main-area">
      <v-container fluid class="main-container pa-3">
        <v-snackbar v-if="messagesEnabled" v-model="errorOpen" class="app-message" color="error" location="bottom" timeout="5000" transition="fade-transition" @after-leave="finishErrorLeave">{{ error }}<template #actions><v-btn variant="text" @click="dismissError">关闭<v-tooltip activator="parent">关闭错误消息</v-tooltip></v-btn></template></v-snackbar>
        <v-snackbar v-if="messagesEnabled && !loading" v-model="noticeOpen" class="app-message" color="info" location="bottom" timeout="4000" transition="fade-transition" @after-leave="finishNoticeLeave">{{ notice }}<template #actions><v-btn variant="text" @click="clearNotice">关闭<v-tooltip activator="parent">关闭状态消息</v-tooltip></v-btn></template></v-snackbar>
        ...
        <v-card v-if="!targetPdf" class="empty-card" variant="tonal" color="primary"><v-card-title>选择 ISO PDF 或整个工程文件夹</v-card-title><v-card-text>系统会先检查可恢复工作区，再在高分辨率 Canvas 中渲染图纸。</v-card-text></v-card>
            ...
            <v-card class="canvas-card" elevation="2">
              <v-toolbar data-tour="canvas-toolbar" density="compact" color="#1c2b35" theme="dark">
                ...
                <div class="page-navigator">
                  <v-btn size="small" icon class="page-group-button" aria-label="显示上一组页码" :disabled="!canShowPreviousPageGroup" @click="stepPageGroup(-1)"><span aria-hidden="true">‹</span><v-tooltip activator="parent">上一组页码</v-tooltip></v-btn>
                  <v-btn v-for="pageNumber in visibleNavigationPages" :key="pageNumber" size="small" class="page-status-button" :class="navigationPageClasses(pageNumber)" :variant="shownPage === pageNumber ? 'flat' : 'tonal'" @click="changePage(pageNumber)">P{{ pageNumber }}</v-btn>
                  <v-btn size="small" icon class="page-group-button" aria-label="显示下一组页码" :disabled="!canShowNextPageGroup" @click="stepPageGroup(1)"><span aria-hidden="true">›</span><v-tooltip activator="parent">下一组页码</v-tooltip></v-btn>
                </div>
                <v-spacer />
              ...
              <div ref="canvasViewport" data-tour="canvas-surface" class="canvas-viewport" :class="{ panning: canvasPanState, 'manual-adding': manualAddMode }" @click="onCanvasClick" @wheel.prevent="onCanvasWheel" @pointerdown="startCanvasPan" @pointermove="moveCanvasPan" @pointerleave="canvasPointerPosition = null" @pointerup="endCanvasPan" @pointercancel="endCanvasPan" @auxclick.prevent>
                <div ref="canvasSurface" class="canvas-surface" :style="canvasSurfaceStyle">
                  <canvas ref="pdfCanvas" class="pdf-canvas"></canvas>
                  <div v-for="marker in referenceDisplayMarkers" :key="marker.key" class="reference-focus-marker" :class="[`reference-focus-marker--${marker.type}`, { 'reference-focus-marker--all': referenceShowAll }]" :style="marker.style" :title="marker.label"><i :style="marker.linkStyle" aria-hidden="true" /><span>{{ marker.label }}</span></div>
    ...
    <v-navigation-drawer data-tour="review" permanent :rail="!reviewDrawerOpen" :rail-width="24" location="right" :width="reviewDrawerWidth" class="review-drawer">
      <button v-if="!reviewDrawerOpen" type="button" class="drawer-edge-toggle drawer-edge-toggle--right" aria-label="打开拓扑标识辅助区域" @click="reviewDrawerOpen = true"><span>‹</span></button>
      <v-card v-else class="review-card" elevation="0">
        <v-card-title class="review-title"><span>拓扑标识</span></v-card-title>
        <template v-if="result">
          ...
          <v-divider />
          <v-card-actions data-tour="review-actions" class="review-actions"><v-btn size="small" @click="requestRenumberAll">全部重编<v-tooltip activator="parent">确认后，每一页分别按画面位置从设置的起始编号开始重新编号</v-tooltip></v-btn><v-btn size="small" :loading="backendSaveState === 'saving'" @click="save">{{ result?.jobId === 'tutorial-000207' ? '保存草稿' : backendSaveButtonLabel }}<v-tooltip activator="parent">普通任务在停止编辑2秒后自动保存到后端；点击可立即保存。教程仅保存浏览器草稿。</v-tooltip></v-btn><v-menu><template #activator="{ props }"><v-btn v-bind="props" size="small" color="primary">导出<v-tooltip activator="parent">选择导出 CSV 或标识 PDF；AI 训练样本由后端在解析完成后自动生成</v-tooltip></v-btn></template><v-list density="compact"><v-list-item title="导出 CSV" @click="exportCsv" /><v-list-item title="导出标识 PDF" @click="exportPdf" /></v-list></v-menu></v-card-actions>
        </template>
        <v-card-text v-else class="review-placeholder">
          <div class="review-placeholder__title">等待识别结果</div>
          <div class="review-placeholder__text">完成识别后，此处辅助查看对象信息、调整外观、重新编号与导出；主要校对操作在中央画布中完成。</div>
        </v-card-text>
      </v-card>
      <button v-if="reviewDrawerOpen" type="button" class="drawer-edge-toggle drawer-edge-toggle--right" aria-label="折叠拓扑标识辅助区域" @click="reviewDrawerOpen = false"><span>›</span></button>
    </v-navigation-drawer>

    <TutorialTour v-model="tutorialOpen" :steps="tutorialSteps" @step-change="handleTutorialStepChange" @close="finishTutorial" />

    <v-dialog v-model="allRenumberConfirmDialog" persistent max-width="500">
      <v-card>
        <v-card-title>确认全部重新编号</v-card-title>
        <v-card-text>将按当前编号规则重新生成全部 {{ activeCandidates.length }} 个有效对象的编号。各页会分别从设定的起始编号开始，此操作会覆盖现有编号，但可以撤销。</v-card-text>
        <v-card-actions><v-btn variant="text" @click="allRenumberConfirmDialog = false">取消</v-btn><v-spacer /><v-btn color="secondary" @click="numberWelds">确认全部重编</v-btn></v-card-actions>
      </v-card>
    </v-dialog>
    ...
    <v-dialog v-model="analysisQueueDialog" max-width="820">
      <v-card class="analysis-queue-card">
        <div class="draft-manager-header analysis-queue-header">
          ...
          <div><v-card-title>解析队列</v-card-title><v-card-subtitle>服务器保留图纸与解析结果，完成后无需再次上传即可恢复核对。</v-card-subtitle></div>
          <v-spacer />
          <v-btn icon size="small" variant="text" :loading="analysisQueueLoading" aria-label="刷新解析队列" @click="refreshAnalysisQueue"><span aria-hidden="true">↻</span></v-btn>
        </div>
        <v-progress-linear v-if="analysisQueueLoading" indeterminate color="secondary" />
        <v-tabs v-model="analysisQueueTab" color="secondary" grow density="compact">
          <v-tab value="current">解析任务</v-tab>
          <v-tab value="archived">已归档（{{ analysisQueue.jobs.filter(job => job.isArchived).length }}）</v-tab>
        </v-tabs>
        <v-card-text class="analysis-queue-content">
          <div class="analysis-queue-summary">
            <v-chip size="small" color="accent" variant="tonal">解析中 {{ analysisQueue.runningCount }}</v-chip>
            <v-chip size="small" color="warning" variant="tonal">等待 {{ analysisQueue.queuedCount }}</v-chip>
            <span>并发上限 {{ analysisQueue.maxConcurrent }}</span>
          </div>
          <v-alert v-if="!analysisQueueLoading && !visibleAnalysisQueueJobs.length" type="info" variant="tonal" density="compact">{{ analysisQueueTab === 'archived' ? '当前没有已归档任务。' : '当前没有解析任务。' }}</v-alert>
          <v-list v-else class="analysis-queue-list" lines="three">
            <v-list-item v-for="job in visibleAnalysisQueueJobs" :key="job.jobId" class="analysis-queue-item">
              ...
              <v-list-item-subtitle>
                <span>{{ job.progressMessage || queueStatusLabel(job) }}</span>
                <v-progress-linear v-if="job.queueState === 'running'" :model-value="queueProgress(job)" color="accent" height="4" rounded class="mt-2" />
              </v-list-item-subtitle>
              <template #append>
                <div class="analysis-queue-actions">
                  <v-chip size="x-small" :color="queueStatusColor(job)" variant="tonal">{{ queueStatusLabel(job) }}</v-chip>
                  <div v-if="job.canReorder" class="analysis-queue-order">
                    <v-btn icon size="x-small" variant="text" :disabled="job.queuePosition <= 1 || analysisQueueActionId === job.jobId" aria-label="提高优先级" @click="moveQueueJob(job, 'up')">↑</v-btn>
                    <v-btn icon size="x-small" variant="text" :disabled="job.queuePosition >= analysisQueue.queuedCount || analysisQueueActionId === job.jobId" aria-label="降低优先级" @click="moveQueueJob(job, 'down')">↓</v-btn>
                  </div>
                  <v-btn v-if="job.canCancel" size="small" color="error" variant="text" :loading="analysisQueueActionId === job.jobId" @click="cancelQueueJob(job)">取消</v-btn>
                  <v-btn v-if="job.canRestore" size="small" color="secondary" variant="tonal" :loading="analysisQueueActionId === job.jobId" @click="restoreQueueJob(job)">恢复核对</v-btn>
                  <v-btn v-if="job.canArchive" size="small" variant="text" :loading="analysisQueueActionId === job.jobId" @click="setQueueJobArchived(job, !job.isArchived)">{{ job.isArchived ? '取消归档' : '归档' }}</v-btn>
                  <v-btn v-if="job.canDelete" size="small" color="error" variant="text" :disabled="result?.jobId === job.jobId" :loading="analysisQueueActionId === job.jobId" @click="requestDeleteQueueJob(job)">删除<v-tooltip activator="parent">{{ result?.jobId === job.jobId ? '当前正在核对此任务，请先切换或关闭当前工作区' : '永久删除任务文件与解析结果' }}</v-tooltip></v-btn>
                </div>
              </template>
            </v-list-item>
          </v-list>
        </v-card-text>
        <v-card-actions class="settings-actions"><v-spacer /><v-btn color="primary" @click="analysisQueueDialog = false">关闭</v-btn></v-card-actions>
      </v-card>
    </v-dialog>

    <v-dialog v-model="analysisQueueDeleteDialog" max-width="460">
      <v-card>
        <v-card-title>确认删除解析任务</v-card-title>
        <v-card-text>将永久删除“{{ analysisQueueDeleteTarget?.fileName || '未命名图纸' }}”的原始文件、对照文件和解析结果，此操作无法撤销。若只是希望减少任务列表冗余，请使用“归档”。</v-card-text>
        <v-card-actions><v-spacer /><v-btn variant="text" @click="analysisQueueDeleteDialog = false">取消</v-btn><v-btn color="error" :loading="analysisQueueActionId === analysisQueueDeleteTarget?.jobId" @click="confirmDeleteQueueJob">永久删除</v-btn></v-card-actions>
      </v-card>
    </v-dialog>

    <v-dialog v-model="systemSettingsDialog" max-width="620">
      <v-card class="settings-card">
        <div class="draft-manager-header settings-header">
          <span class="draft-manager-header__icon settings-header__icon" aria-hidden="true">⚙</span>
          <div><v-card-title>系统设置</v-card-title><v-card-subtitle>管理界面、草稿、渲染性能和快捷操作。</v-card-subtitle></div>
        </div>
        <v-card-text class="settings-content">
          <div class="settings-section">
            ...
            <v-switch v-model="reviewDrawerOpen" color="secondary" hide-details label="显示拓扑标识辅助区域" />
            <v-switch v-model="tooltipsEnabled" color="secondary" hide-details label="启用全局 Tooltip 提示" />
            <v-switch v-model="messagesEnabled" color="secondary" hide-details label="显示操作与错误消息" />
            <v-text-field v-model.number="pageButtonsPerGroup" class="mt-3" type="number" min="1" max="20" label="每组最多显示页码按钮数" hint="默认 5 个；超出后通过页码按钮两侧的箭头切换分组" persistent-hint />
          </div>
          <div class="settings-section">
          ...
          <div class="settings-section">
            ...
            <v-select v-model="canvasPerformanceMode" :items="canvasPerformanceOptions" label="清晰度与内存档位" hide-details class="mb-3" />
            <div class="canvas-profile-summary"><v-chip size="small" color="secondary" variant="tonal">{{ canvasPerformanceProfile.label }}</v-chip><span>单画布最高 {{ Math.round(canvasPerformanceProfile.maxPixels / 1_000_000) }} 百万像素 · 位图缓存约 {{ Math.round(canvasPerformanceProfile.cacheBytes / 1024 / 1024) }} MB</span></div>
            <div class="shortcut-settings__hint mt-2">自动模式依据设备内存、CPU 核心数和触控屏尺寸选择；也可手动固定档位，选择后立即生效。</div>
          </div>
          <div class="settings-section">
            <div class="section-label">标识默认外观</div>
            <div class="marker-appearance-card settings-marker-appearance">
              <div class="marker-appearance-tabs" role="tablist" aria-label="默认标识外观分类">
                <button v-for="group in markerAppearanceGroups" :key="group.key" type="button" role="tab" :aria-selected="settingsAppearanceTab === group.key" :class="{ active: settingsAppearanceTab === group.key }" @click="settingsAppearanceTab = group.key"><i :style="{ backgroundColor: group.style.color }" />{{ group.title.replace('标识', '') }}</button>
              </div>
              <div class="marker-appearance-panel" role="tabpanel">
                <div class="marker-appearance-heading"><span><i :style="{ backgroundColor: activeSettingsAppearanceGroup.style.color }" /><strong>{{ activeSettingsAppearanceGroup.title }}</strong></span><small>{{ activeSettingsAppearanceGroup.subtitle }}</small></div>
                <div class="marker-appearance-form">
                  <div class="parameter-grid"><v-select v-model="activeSettingsAppearanceGroup.style.shape" :items="shapeOptions" label="外形" /><v-text-field v-model.number="activeSettingsAppearanceGroup.style.frameSize" type="number" min="18" max="64" label="框尺寸" /></div>
                  <div class="parameter-grid mt-3"><v-text-field v-model.number="activeSettingsAppearanceGroup.style.fontSize" type="number" min="7" max="24" label="字号" /><v-text-field v-model="activeSettingsAppearanceGroup.style.color" type="color" label="颜色" /></div>
                </div>
              </div>
            </div>
          </div>
          ...
          <v-btn block color="primary" variant="tonal" class="settings-config-button" @click="systemSettingsDialog = false; symbolConfigDialog = true">打开焊口符号研究配置</v-btn>
        </v-card-text>
        <v-card-actions class="settings-actions"><v-spacer /><v-btn color="primary" @click="systemSettingsDialog = false">完成</v-btn></v-card-actions>
      </v-card>
    </v-dialog>
    ...
    <v-dialog v-model="draftManagerDialog" max-width="720">
      <v-card class="draft-manager-card">
        <div class="draft-manager-header">
          ...
          <v-spacer />
          <v-btn v-if="draftEntries.length" size="small" color="secondary" :variant="draftSelectionMode ? 'flat' : 'tonal'" @click="toggleDraftSelectionMode">{{ draftSelectionMode ? '退出多选' : '多选' }}</v-btn>
        </div>
        ...
        <v-card-text>
          ...
          <template v-else>
            <div v-if="draftSelectionMode" class="draft-manager-toolbar">
              <v-checkbox-btn :model-value="allDraftsSelected" :indeterminate="selectedDraftKeys.length > 0 && !allDraftsSelected" density="compact" color="secondary" aria-label="选择全部草稿" @update:model-value="toggleAllDrafts" />
              <v-btn size="small" variant="text" color="secondary" @click="toggleAllDrafts(!allDraftsSelected)">{{ allDraftsSelected ? '取消全选' : '全选' }}</v-btn>
              <span>{{ selectedDraftKeys.length ? `已选择 ${selectedDraftKeys.length} 项` : `共 ${draftEntries.length} 个草稿` }}</span>
              <v-spacer />
              <v-btn size="small" color="error" variant="tonal" :disabled="!selectedDraftKeys.length" @click="draftBatchDeleteDialog = true">删除所选</v-btn>
            </div>
            <v-list class="draft-manager-list" lines="three">
            <v-list-item v-for="entry in draftEntries" :key="entry.key" :class="{ 'draft-manager-list__selected': selectedDraftKeys.includes(entry.key) }" @click="draftSelectionMode && toggleDraftSelection(entry.key)">
              <template #prepend><div class="draft-manager-prepend"><v-checkbox-btn v-if="draftSelectionMode" v-model="selectedDraftKeys" :value="entry.key" density="compact" color="secondary" :aria-label="`选择草稿 ${entry.payload?.fileName || '未命名图纸'}`" @click.stop /><span class="draft-manager-file-icon" aria-hidden="true">PDF</span></div></template>
              ...
              <v-list-item-subtitle>{{ draftSavedAt(entry) }} · {{ draftCandidateCount(entry) }} 个标识 · 对照 PDF {{ entry.payload?.referenceFiles?.length || 0 }} 份</v-list-item-subtitle>
                  <template #append>
                    <div class="draft-manager-actions">
                      ...
                      <v-chip v-else-if="entry.archiveStatus === 'partial'" size="x-small" color="warning" variant="tonal">容灾归档未完整</v-chip>
                      <template v-if="!draftSelectionMode"><v-btn size="small" color="secondary" variant="tonal" :disabled="!canOpenManagedDraft(entry)" @click.stop="openManagedDraft(entry)">打开</v-btn><v-btn size="small" color="error" variant="text" @click.stop="draftDeleteTarget = entry">删除</v-btn></template>
                </div>
              </template>
            </v-list-item>
            </v-list>
    ...
    <v-dialog v-model="draftBatchDeleteDialog" persistent max-width="460">
      <v-card>
        <v-card-title>确认批量删除草稿</v-card-title>
        <v-card-text>确定删除选中的 {{ selectedDraftKeys.length }} 个浏览器草稿吗？删除后无法恢复。</v-card-text>
        <v-card-actions><v-btn variant="text" :disabled="draftBatchDeleting" @click="draftBatchDeleteDialog = false">取消</v-btn><v-spacer /><v-btn color="error" :loading="draftBatchDeleting" @click="deleteSelectedDrafts">删除所选</v-btn></v-card-actions>
      </v-card>
    </v-dialog>
